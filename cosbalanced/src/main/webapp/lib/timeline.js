...
(function()
{
	...
	function init(obj)
	{
		...
		return obj.table ? true : id.error(me+'Can\'t init query timeline!');
	}
	...
	function Table(obj)
	{
			...
			function htmlOf(columns)
			{
				...
				for (var i = 0; i < columns.length; i++) {
					if (columns[i].type != COS.index) {
						...
						cache += '<th id="'+columns[i].id+'" class="'+ Class.hidden +' index"></th>';
					}
				}
		...
		function addRows(obj)
		{
			...
			if (obj.rows && obj.rows.length) {
				for (var i = 0, rows = obj.rows; i < rows.length; i++) {
					...
					options.parent = getParentRow(obj, rows[i+1]);
		...
		function getParentRow(obj, nextRow)
		{
			...
			{
				if (columns[i].type == COS.index) {
					nextIndex = id.cos.valueOf(nextRow[i]); 
					nextIndex = nextIndex.split('.');
										
					if (parseInt(nextIndex.splice(-1)) == 1) {
						return true;
					}
				}
			}
		...
		function addRow(obj, row, options)
		{
			...
			$tr = $('<tr class="'+ Class.data + (options.last ? ' '+Class.last : '') + (index ? ' '+Class.index +'-'+ index : '') + (options.parent ? ' '+ Class.parent : ' '+ Class.child)+'">');
				...
				if (columns[i].type != COS.index) {
					...
					switch (cell.type)
					{
						...
						{	
							...
							if (parseInt(index) > 1) {
								addIndexButton(obj, columns, row, $td, $div, options);
							}
						...
						case COS.text: 
						{
							$span = $('<span>'+ (cell.value ? id.cos.valueOf(cell) : '') +'</span>')
							$div.append($span); 
							$td.attr(Data.key, cell.value);
							$td.append($div); 
		...
		function addIndexButton(obj, columns, row, $td, $div, options) {
			options = options || {};
			var indexCell, $div;
			var $toggle = $('<div class="'+ Class.toggle +'"></div>');
			
			if (columns[0].type == COS.index) {
				indexCell = merge(columns[0], row && row[0] || {});
				id.object.bind(indexCell, obj.getOwner(), $td);
				addEvents($toggle, indexCell, options); 
				$div.prepend($toggle);
			}
		}
		...
		function selectRow($td) {
			var $rows = $td.parents('.' + Class.isTimeline).find('tr.'+ Class.data);
			var $row = $td.parents('tr.' + Class.data);
	...
	function addTimeline(column, cell)
	{
		...
		var map = { 'nodata': 0, 'grey': 1, 'yellow': 2, 'orange': 3, 'whiteWithBorder': 4, 'red': 5, 'green': 6, 'orangeWithBorder': 7, 'blank': 8 };
	...
	function getItemPosition(cell, item) {
		...
		//if (item && !id.isMember([Status.blank, Status.orangeWithBorder, Status.whiteWithBorder], item.status)) corr = 1;
		...
		var itemStart = getTime(item && item.from || cell.from) - corr;
	...
	{	
		...
		{ 
			...
			for (col = 0; col < n; col++)
			{
				...
				$cp = $columns.children(/*td*/).not('.index').eq(col);
	...
	function getTime(dateTime) {
		...
		var dateObject = new Date(dateArray[2], dateArray[1] - 1, dateArray[0], timeArray[0], timeArray[1], 0);

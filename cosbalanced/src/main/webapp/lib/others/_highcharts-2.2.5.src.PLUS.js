			...
			for (key in hash) {
				...
				if (result !== false) {
					...
					if (wrapper.symbolName && /^(x|y|width|height|r|start|end|innerR|anchorX|anchorY)/.test(key)) {
						...
						if (!hasSetSymbolSize && wrapper.r > 0) { //PATCH Information Design line scatter symbol like candlestick (Bug Redraw)
...
SVGRenderer.prototype = {
	...
	symbols: {
		...
		// PATCH Information Design 06.2012 Line Scatter Symbols like Candlestick
		'hline': function (x, y, w) {
			var symbolWidth = (w - 2 * 0.2)/2;
			return [
				M,x+symbolWidth,y,
				L,x-symbolWidth,y
			];
			...
			while (i--) {
				...
				if (plotY !== UNDEFINED && !isNaN(plotY)) {
					...
					if (graphic) { // update
						...
						.attr(pointAttr)
						.add(series.group);
					} else if (symbol == 'hline') {	// Patch Information Line Design Scatter Symbol like Candlestick	
						
						// Patch Information Design Line Scatter Symbol like Candlestick	
						var symbolSize = series.xAxis.translationSlope / 2;
		...
		if (options.enabled || series._hasPointLabels) {
			...
			each(points, function (point) {
				...
				// in the point options, or if they fall outside the plot area.
					...
					// Hide labels outside the plot area 
					if (series.isCartesian) {

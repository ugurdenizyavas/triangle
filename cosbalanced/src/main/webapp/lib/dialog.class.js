...
(function()
{
	...
	{
		classify({name:'Dialog', definition:function()
		{
			...
			def({ name:'resetMessages', method: function resetMessages(obj)
			{
			    id.gui.message.clearAll(obj);
			}});

			def({ name:'setResetDynamicByUpdate', method: function setResetDynamicByUpdate(flag)
			{
			    prefs.resetDynamicByUpdate = flag;
			}});
			...
			function set(obj)
			{
				...
				for (var i = 0; obj.items && i < obj.items.length; i++)
				{
					...
					switch (item.type)
					{
						...
						case cos.check      : id.gui.flag.init(item); break;
	...
	id.gui.dialog = new id.gui.Dialog(my.name);

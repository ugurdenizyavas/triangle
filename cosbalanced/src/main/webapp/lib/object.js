...
(function()
{
	...
	id.object = 
	{
		...
		leave: function(causedBy, options) { return cursor ? parts[cursor].leave(causedBy, options) : false; },
	...
	function Part(aObj, options)
	{
		...
		function leave(causedBy, options)
		{
			...
			if (causedBy)
			{
				...
				if (options.cause.url.indexOf(from.getIndex()) !== -1)
				{
				...
				if (causedBy.isWidget() || (options.cause && options.cause.trigger == 'timer'))
				{
					// The causer of exit is a process (timer).
